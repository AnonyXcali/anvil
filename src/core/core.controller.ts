...
export class CoreController {
  ...
  @Post()
  async Initiate(
    @Body() body: { query: string; user_id: string },
    @Session() session: UserSession,
  ) {
    return this.coreService.handleFlowInitiation(body.query, session.user.id);
  }

  @Post('/talk')
  async Converse(@Body() body: { query: string; conversation_id: string }) {
    return this.coreService.talk(body.query, body.conversation_id);
  }
